...
with col_title:
    st.markdown("<div class='logo'><h2 style='margin:0'>Hillani</h2> <span class='muted'>Rhinoplasty Photo Measurements</span></div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Mobile-friendly • Works on phones when deployed on Streamlit Cloud")
...
if st.button("🚀 Auto-detect view + landmarks + measurements"):
    try:
        res = run_facemesh(img)
        if not res.multi_face_landmarks:
            st.error("No face landmarks detected. Try a clearer image.")
        else:
            lms = res.multi_face_landmarks[0].landmark
            idx = {"nose_tip":1,"nasion":168,"left_eye":33,"right_eye":263,"nose_base":2,"alar_left":94,"alar_right":326}
            def xy(i): 
            ...
            st.session_state._auto = dict(View_Auto="Frontal-like", Nasal_Length_px=nasal_len, Tip_Projection_px=proj, Goode_Ratio=goode)
...
if st.button("💾 Save results"):
    ...
    auto = st.session_state.get("_auto", {})
    row = dict(
